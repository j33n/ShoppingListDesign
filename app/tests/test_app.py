...
class FlaskTestCase(unittest.TestCase):
    ...
    def test_homepage(self):
        ...
        self.assertTrue(
            'Keep track of your shopping' in response.get_data(as_text=True))
    ...
    def test_user_signup(self):
        ...
        response = self.accounts(
            'Pacito', 'pacito@test.com', 'secret', 'secret')
    ...
    def test_wrong_data(self):
        ...
        response1 = self.accounts(
            'uwouwo', 'uwouwo.test.com', 'secret', 'secret')
        self.assertIn(b'Invalid email address', response1.data)
        response2 = self.accounts(
            'Puerto', 'uwouwo@test.com', 'pokito', 'pokito1')
        self.assertIn(b'Passwords must match', response2.data)
        response3 = self.accounts(
            'Rico', 'uwouwo@test.com', 'pacito', 'pacito')
        self.assertIn(
            b'Field must be between 5 and 50 characters', response3.data)
    ...
    def test_user_logout(self):
        ...
        self.assertTrue(
            b'We hope you enjoyed organizing and sharing lists see you soon' in response.data)

    def test_user_added_to_store(self):
        """Test users is added to our store"""
        users_size = len(Store().users)
        response = self.accounts(
            'Despacito', 'despacito@test.com', 'secret', 'secret')
